...
//! drive the app the same way. Keeping this wiring in the library (rather than
//! inline in `main`) is what lets `tests/` boot a real server in-process.
...
impl FromRef<AppState> for UserConnectionRegistry {
    fn from_ref(state: &AppState) -> Self {
...
/// Connect to SurrealDB + Redis, wire repositories / room managers / collab,
/// and assemble the shared [`AppState`].
///
/// The Prometheus handle is supplied by the caller because the global metrics
/// recorder may only be installed once per process — `main` installs it, while
/// tests pass a non-installed handle.
...
/// Build the fully-layered Axum router for the given state. The returned router
/// still needs `into_make_service_with_connect_info::<SocketAddr>()` at serve
/// time so IP-based rate limiting can read the peer address.
pub fn build_router(state: AppState) -> Router {
    ...
    let api_routes = Router::new()
        ...
        .route(
            "/api/dms",
            get(handlers::direct::list_direct_messages).post(handlers::direct::open_direct_message),
        )
        // Follow routes
        .route(
        ...
        .route(
            ...
            post(handlers::servers::leave_server),
        )
        // Channel routes
        .route(
